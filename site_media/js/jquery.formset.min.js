...
options.formTemplate=template;if($$.attr('tagName')=='TR'){var numCols=$$.eq(0).children().length,buttonRow=$('<tr><td colspan="'+numCols+'"><a class="'+options.addCssClass+'" href="javascript:void(0)">'+options.addText+'</a></tr>').addClass(options.formCssClass+'-add');$$.parent().append(buttonRow);if(hideAddButton)buttonRow.hide();addButton=buttonRow.find('a');}else{$$.filter(':last').after('<a class="'+options.addCssClass+'" href="javascript:void(0)">'+options.addText+'</a>');addButton=$$.filter(':last').next();if(hideAddButton)addButton.hide();}
addButton.click(function(){var formCount=parseInt(totalForms.val()),row=options.formTemplate.clone(true).removeClass('formset-custom-template'),buttonRow=$($(this).parents('tr.'+options.formCssClass+'-add').get(0)||this);applyExtraClasses(row,formCount);row.insertBefore(buttonRow).show();row.find(childElementSelector).each(function(){updateElementIndex($(this),options.prefix,formCount);});totalForms.val(formCount+1);if(!showAddButton())buttonRow.hide();if(options.added)options.added(row);return false;});}
return $$;}
$.fn.formset.defaults={prefix:'form',formTemplate:null,addText:'Agregar otro',deleteText:'Quitar',addCssClass:'add-row slim_button',deleteCssClass:'delete-row',formCssClass:'dynamic-form',extraClasses:[],keepFieldValues:'',added:null,removed:null};})(jQuery)
